...
from schemas.Damage import DamageCreate, DamageResponse
...
async def read_damage(damage_id: int, db: AsyncSession = Depends(get_db)):
...
async def read_damages(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    return await get_damages(db, skip=skip, limit=limit)


@router.put("/{damage_id}/status", response_model=DamageResponse)
async def change_damage_status(damage_id: int, new_status: str, db: AsyncSession = Depends(get_db)):
...
@router.delete("/{damage_id}", response_model=DamageResponse)
async def remove_damage(damage_id: int, db: AsyncSession = Depends(get_db)):
